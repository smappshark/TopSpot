jQuery(document).ready(function() {
	...
	function getFactSheetData() {
		...
		jQuery.get('ActionServlet', {
			...
			jQuery(".loader").hide();
			jQuery(".js_trigger").show();
			responseJson = JSON.parse(responseJson);
			// Binding ResponseJSON to Fact sheet - Start
			var htmlString = "";
			for (var i = 0; i < responseJson.length; i++) {
				var table = "<table class='investor_tbl' cellspacing='0' width='70%' cellpadding='3'></tbody>";
					...
					table += "<tr><td align='right' colspan='4'><label class='submit' onclick='js_buildingDetails_click(this)'>CLick </label></td></tr>";
					table += "<tr style='display:none'><td colspan='4'>" + responseJson[i].id + "</td></tr>";
					table += "</tbody></table>";
